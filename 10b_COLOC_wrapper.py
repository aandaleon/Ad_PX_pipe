...
for pheno_name in pheno_names:
    for tissue, tissue_sample_size in zip(tissues, tissues_sample_size):
        run_COLOC = "python3 summary-gwas-imputation/src/run_coloc.py -gwas COLOC_input/" + prefix + "_" + pheno_name + "_GWAS_" + tissue + ".txt.gz -gwas_sample_size " + GWAS_sample_size + " -eqtl COLOC_input/" + prefix + "_" + pheno_name + "_eQTL_" + tissue + ".txt.gz -eqtl_sample_size " + tissue_sample_size + " -output COLOC_results/" + prefix + "_" + pheno_name + "_" + tissue + ".txt.gz" #COLOC prints out too much so throw it into a file

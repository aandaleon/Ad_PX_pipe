...
sig_snp = open("output/" + input_prefix + "_sig_snps.txt", "w")
sig_snp.write("chr\trs\tps\tn_miss\tallele1\tallele0\taf\tbeta\tse\tl_remle\tl_mle\tp_wald\tp_lrt\tp_score\tpheno\n")
sig_gene = open("output/" + input_prefix + "_sig_genes.txt", "w")
...
for pheno_name in pheno_names:
    for chrom in range(1, 23): #find sig snp
        for line in open("output/" + input_prefix + "_" + pheno_name + "_chr" + str(chrom) + ".assoc.txt", "r"):
    ...
    for tissue in tissues: #find sig gene
        for line in open("output/" + input_prefix + "_" + pheno_name + "_" + tissue + ".assoc.txt", "r"):
            arr = line.strip().split()
            (chr, rs, ps, n_miss, allele1, allele0, af, beta, se, l_remle, l_mle, p_wald, p_lrt, p_score) = arr[0:14]
...
sig_snp.close()
sig_gene.close()
print("Completed writing output/" + input_prefix + "_sig_snps.txt and output/" + input_prefix + "_sig_genes.txt")

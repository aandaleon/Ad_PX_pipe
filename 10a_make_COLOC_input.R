...
parser$add_argument("--ma", help = ".ma file (the input of GCTA-COJO)")
...
for(tissue in 1:length(tissues)){ #read in tissue's .frq file for MAF
  ...
  for(chr in chrs){ #yes triple loops are ratchet
    GEMMA_result <- fread("output/" %&% GWAS_prefix %&% "chr" %&% chr %&% ".assoc.txt", header = F, nThread = 30)
  ...
  snps_in_all <- intersect(snps_in_both, GWAS_SNPs)
  ...
  fwrite(eQTL_write, "COLOC_input/" %&% GWAS_prefix %&% "eQTL_" %&% tissues[tissue] %&% ".txt", quote = F, sep = "\t", na = "NA", row.names = F, col.names = T)
  gzip("COLOC_input/" %&% GWAS_prefix %&% "eQTL_" %&% tissues[tissue] %&% ".txt", destname = "COLOC_input/" %&% GWAS_prefix %&% "eQTL_" %&% tissues[tissue] %&% ".txt.gz", overwrite = T) #script may only take .gz values so can't hurt to be too careful
  fwrite(GWAS_write, "COLOC_input/" %&% GWAS_prefix %&% "GWAS_" %&% tissues[tissue] %&% ".txt", row.names = F, col.names = T, sep = "\t", quote = F, na = "NA")
  gzip("COLOC_input/" %&% GWAS_prefix %&% "GWAS_" %&% tissues[tissue] %&% ".txt", "COLOC_input/" %&% GWAS_prefix %&% "GWAS_" %&% tissues[tissue] %&% ".txt.gz", overwrite = T)
  print("Completed with " %&% tissues[tissue] %&% ".")
}
